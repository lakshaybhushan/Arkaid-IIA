...
### steam_games
```sql
SELECT movies, name AS game_name, id
FROM steam_games
```

### modders
```sql
SELECT type_of_mod, primary_game, country AS modder_country, name AS modder_name
FROM modders
```

## Results Preview
| movies   | game_name   | id   | type_of_mod   | primary_game   | modder_country   | modder_name   |
|----------|-------------|------|---------------|----------------|------------------|---------------|