...
class PlayersWarehouseManager:
    ...
    def _create_new_view(self, cursor):
        ...
        for source in self.mapping_config["sources"]:
            ...
            # Add UUID generation for player_id at the start
            select_columns.append("gen_random_uuid() as player_id")
            
            # Get target columns in the order they appear in target_schema
            target_columns = [
                col for col in self.mapping_config["target_schema"].keys() 
                if col != 'player_id'  # Skip player_id as we're generating it
            ]
            
            # For each target column, find its corresponding source column
            for target_col in target_columns:
                ...
                if target_col == "player_library_count":
                    # Special handling for library count
                    select_columns.append(f"""
                        COALESCE(
                            CAST(array_length(
                                string_to_array(NULLIF(library, ''), ','),
                                1
                            ) AS INTEGER),
                            0
                        ) as player_library_count
                    """)
                elif source_col:
            ...
            # Add hardcoded platform column based on source
            platform_name = 'Steam' if source["name"] == 'steam_players' else 'Epic'
            select_columns.append(f"'{platform_name}'::TEXT as player_platform")
        ...
        # Create a unique index on player_id
        cursor.execute(f"""
        CREATE UNIQUE INDEX {self.mapping_config['materialized_view']}_pkey 
        ON {self.mapping_config['materialized_view']} (player_id);
        """)
    ...
    def _setup_foreign_servers(self):
        ...
        try:
            ...
            for source in self.mapping_config["sources"]:
                ...
                cursor.execute(f"CREATE SCHEMA IF NOT EXISTS {source['database'].lower()};")
...
if __name__ == "__main__":
    manager = PlayersWarehouseManager(
        db_config_path='db_config.yaml',
        mapping_file_path='mappings/mv_players.json',
        schema_mapping_path='mappings/mv_players_schema_mappings.json'
    )
    
    try:
        manager.create_materialized_view()
        print("Successfully managed players materialized view!")
    except Exception as e:
        print(f"Error: {str(e)}")
    finally:
