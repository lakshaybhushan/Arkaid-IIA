...
def load_mapping():
    ...
    mapping_path = 'dags/mappings/content_creator_mappings.json'
    ...
    try:
        with open(mapping_path, 'r') as file:
            ...
            logger.info("\nColumn Mappings:")
            for source_col, dest_col in mapping['column_mappings'].items():
                logger.info(f"  {source_col} -> {dest_col}")
            logger.info("================================")
            
            # Log the complete mapping JSON for reference
            logger.info("Complete Mapping JSON:")
...
def generate_materialized_view_sql():
    ...
    logger.info(f"Column mappings: {json.dumps(column_mappings, indent=2)}")
    ...
    for dest_col, source_col in column_mappings.items():
        ...
        logger.debug(f"Added column mapping: {source_col} -> {dest_col}")
...
# Create the DAG
dag = DAG(
    'materialized_view_content_creators',
    default_args=default_args,
    description='Creates and refreshes materialized view for content creators',
    schedule_interval='@daily',
    catchup=False
)

logger.info("Creating DAG: materialized_view_content_creators")

# Task to create materialized view if not exists
