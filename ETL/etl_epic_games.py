...
def create_table_query(df: pd.DataFrame, table_name: str) -> str:
    ...
    CREATE INDEX idx_{table_name}_game_id ON {table_name}(game_id);
    """
...
def main():
    ...
    try:
        # Connect to database - Note the change to DB2
        ...
        df.drop_duplicates(subset=['game_id'], inplace=True, keep='first')
